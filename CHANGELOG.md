...
- The badge-committing CI job is removed. It pushed regenerated SVGs back to
  ...
  Badges are generated by the Pages build and served from the site, which also
  stops a committed badge from disagreeing with the current score between runs.
  `static/badges/` is deleted.
